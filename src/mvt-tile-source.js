 ...
 *     { style: defaultStyle(), workerLibs: { optional CDN overrides } }
 *   );
 ...
 */
$.MVTTileJSONTileSource = class extends $.TileSource {
    constructor({ template, scheme = 'xyz', tileSize = 512, minLevel = 0, maxLevel = 14, width, height, extent = 4096, style, workerLibs }) {
        super({ width, height, tileSize, minLevel, maxLevel });
        this.template = template;
        this.scheme = scheme;
        this.extent = extent;
        this.style = style || defaultStyle();
        this._worker = makeWorker(workerLibs);
        ...
        this._worker.onmessage = (e) => {
            ...
            const p = this._pending.get(msg.key);
            if (!p) {
                return;
            }
            ...
            if (msg.ok) {
                ...
                p.fail(msg.error || 'Worker failed');
            }
        };
    ...
    configure(data, dataUrl, postData) {
        ...
        return {
            ...
            workerLibs: undefined,  // todo local worker libs
        };
    }
    ...
    downloadTileStart(context) {
        ...
        this._worker.postMessage({ type: 'tile', key: key, z: tile.level, x: tile.x, y: tile.y, url: context.src });
    }
};
